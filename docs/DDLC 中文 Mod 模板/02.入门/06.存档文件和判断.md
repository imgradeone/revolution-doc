...
一般来说，在使用 Ren'Py 制作的游戏的标准主存档位置里会有一个名为 `DDLCModTempCNNext` 的文件夹，那就是模组模板的存档位置。
里面通常有一个名为 `persistent` 的文件，那就是我们今天要重点介绍的存档文件。
...
来记录玩家游玩到了哪一个周目（虽然你可能用不到）。一般情况下为了保证兼容性，请保持数值范围在 0 到 4 之间。
