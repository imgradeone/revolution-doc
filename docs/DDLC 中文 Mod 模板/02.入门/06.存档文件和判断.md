...
今天学的内容会有一点点的难，但它可以为 Mod 增加更多的乐趣。
...
现在，您进入到了使用 Ren'Py 制作的游戏的标准存档位置。一般来说，您在目录里会看到一个名为 `DDLCModTempCNNext` 的文件夹，那就是模组模板的存档位置。
（您可以去 `options.rpy` 更改存档文件夹名称。）
...
来记录玩家游玩到了哪一个周目（虽然你可能用不到）。为保证兼容性，一般情况下，请保持数值范围在 1 到 4 之间。
...
- `elif <条件>: ...` 代表如果 if 链里的以上条件不能被满足，但这里定义的条件能够被满足，就执行某些语句。
...
- `else: ...` 代表如果 if 链里的以上条件不能被满足，就执行某些语句。
...
执行上面的代码，会发现如果纱世里好感度较高（在选择吃什么时选择了小饼干），在后面纱世里就会说"诶嘿嘿~"，否则只会继续。
