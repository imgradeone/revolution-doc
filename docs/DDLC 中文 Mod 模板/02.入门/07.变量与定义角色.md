...
在电子游戏中，变量是一个非常重要的部分。Python 里的变量可以存储各种值：布尔值、整数值、字符串... 这些在之前存档文件那一章也有稍微提到过。
...
运行脚本，如果 s_aff 大于或等于 1，就会触发Sayori支线；
否则如果 n_aff 大于或等于 1，就会触发Natsuki线。
你可能也发现且在这两条线路中，Sayori 和 Natsuki都会说出MC的名字
