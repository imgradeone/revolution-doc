...
再次运行 Mod，会发现角色立绘会有部分图像在屏幕外\角色立绘过于挨近用户，`zorder`的用法就体现在这里了：
...
运行Mod，会发现这时候角色立绘不会过于的挨近屏幕，所以`zorder`的作用就是控制角色在图层z轴上的位置。

::: tip 提示：

`zorder`后面的数值越大，角色立绘就越挨近用户，反之数值越小，离用户越远。
...
如果在说话时，我们需要改变角色的表情，是不是需要一直重复执行`show`与`hide`呢？当然不用，我们只需要在句子前加上对应的表情就可以了，如下例：
...
运行Mod 我们会发现在Sayori说“耶！小饼干！”时，Sayori脸上的表情就会变化了。更多表情请自行拆解images.rpa。
